...
class TiagoIroningEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, inFileStr='map1.csv', initX=2, initY=2):
        ...
        optionsH.put('remote','/tiagoSim/head')
        optionsH.put('local','/alma/tiagoSim/head')
        self.ddH = yarp.PolyDriver(optionsH)
        if not self.ddH.isValid():
            print('[error] Cannot connect to: /tiagoSim/head')
            quit()
        ...
        optionsT.put('remote','/tiagoSim/trunk')
        optionsT.put('local','/alma/tiagoSim/trunk')
        self.ddT = yarp.PolyDriver(optionsT)
        if not self.ddT.isValid():
            print('[error] Cannot connect to: /tiagoSim/trunk')
            quit()
        ...
        optionsRA.put('remote','/tiagoSim/rightArm')
        optionsRA.put('local','/alma/tiagoSim/rightArm')
        self.ddRA = yarp.PolyDriver(optionsRA)
        if not self.ddRA.isValid():
            print('[error] Cannot connect to: /tiagoSim/rightArm')
            quit()
        ...
        optionsCCTRA.put('cartesianRemote', '/tiagoSim/trunkAndRightArm/CartesianControl')
        optionsCCTRA.put('cartesianLocal', '/alma/tiagoSim/trunkAndRightArm/CartesianControl')
        self.ddccTRA = yarp.PolyDriver(optionsCCTRA)
        if not self.ddccTRA.isValid():
            print('[error] Cannot connect to: /tiagoSim/trunkAndRightArm/CartesianControl')
            quit()
        ...
        try:
            ...
            print('TiagoIroningEnv.__init__: full exception message:', e)
            print('TiagoIroningEnv.__init__: init out of bounds, please review code!')
            quit()
    ...
    def step(self, action):
        #print('TiagoIroningEnv.step', action)
        ...
        try:
            ...
            # state preserved
            print('TiagoIroningEnv.step: full exception message:', e)
            print(
                'TiagoIroningEnv.step: probably went out of bounds, add some walls on your map!')
            terminated = True
            quit()
        ...
        if candidate_state_tag == 0:  # void
            ...
            print('TiagoIroningEnv.step: found wicked tag, please review!')
            terminated = True
            quit()
        ...
        if not np.any(self.inFile == 2):
            print('TiagoIroningEnv.step: done yay!')
            reward = 1.0
            terminated = True
    ...
    def render(self):
        #print('TiagoIroningEnv.render', self.render_mode)
